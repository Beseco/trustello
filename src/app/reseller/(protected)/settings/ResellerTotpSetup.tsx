...
import {
  generateResellerTotpSecret,
  enableResellerTotp,
  disableResellerTotp,
} from "@/server/actions/reseller-settings";
import { Input } from "@/components/ui/input";

type Props = { totpEnabled: boolean };
type Step = "idle" | "setup" | "confirm";

export function ResellerTotpSetup({ totpEnabled }: Props) {
  ...
  function handleStartSetup() {
    ...
    startTransition(async () => {
      const result = await generateResellerTotpSecret();
  ...
  function handleEnable() {
    ...
    setError(null);
    startTransition(async () => {
      const result = await enableResellerTotp(secret, code.trim(), password);
      if (result.error) {
  ...
  function handleDisable() {
    ...
    setError(null);
    startTransition(async () => {
      const result = await disableResellerTotp(password);
      if (result.error) {
        setError(result.error);
      } else {
        setSuccess(true);
      ...
      <div className="space-y-4">
        ...
        {step === "idle" ? (
          ...
          <div className="space-y-3">
            <p className="text-sm text-muted-foreground">Geben Sie Ihr Passwort ein, um 2FA zu deaktivieren:</p>
            <Input
              type="password"
  ...
  return (
    <div className="space-y-4">
          ...
          <div className="flex flex-col items-start gap-4 sm:flex-row">
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center gap-2">
                ...
                <button onClick={copySecret} className="text-muted-foreground hover:text-foreground" title="Kopieren">
      ...
      {step === "confirm" && (
        <div className="space-y-3">
          <p className="text-sm text-muted-foreground">
            6-stelligen Code eingeben, um die Einrichtung zu bestätigen:
          </p>
          <Input
            type="text"
            inputMode="numeric"
